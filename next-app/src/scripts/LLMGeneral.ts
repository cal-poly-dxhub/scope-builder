...
import { gen_prompt, read_prompt } from "@/constants/prompt";
...
const getBedrockResponse = async (
  ...
  console.warn("not doing requests right now (remove when ready)");
  return [{ type: "text", text: "An error occurred" }];
  try {
    const body = JSON.stringify({
      messages,
    });
    const url = process.env.NEXT_PUBLIC_LAMBDA_ENDPOINT as string;
    const options = {
      method: "POST",
      headers: {
