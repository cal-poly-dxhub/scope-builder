...
const useBedrockWebSocket = () => {
  ...
  const restEndpoint =
    "https://b0pcz6pe68.execute-api.us-west-2.amazonaws.com/prod/api/async/search";
  const wssEndpoint =
    "wss://rabqnoi31j.execute-api.us-west-2.amazonaws.com/prod/";
  ...
  const startBedrockProcess = async (userInput: string) => {
    ...
    try {
      ...
      const response = await axios.post(restEndpoint, message, {
  ...
  const { lastJsonMessage, readyState } = useWebSocket(
    connectionID && wssEndpoint
      ? `${wssEndpoint}?execution_arn=${connectionID}`
      : null,
    {
